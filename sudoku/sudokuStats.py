...
gStats = {"bruteForce": 0, "totalDots": 0, "noChoices": 0, "maxChoices": 0}
...
def readFile(pzls):
    ...
    for i, pzl in enumerate(pzls):
        ...
        if gStats["maxChoices"] == 9 and not hasMax:
            print(i + 1,":", pzl)
...
#solves the puzzle
def solvePuzzle(pzl):
    gStats["totalDots"] += len([ch for ch in pzl if ch == '.'])
    return bruteForce(pzl, -2)
...
def findBestPos(pzl, loc):
    global gStats, hasMaxSym
    indexes = [i for i, letter in enumerate(pzl) if letter == '.'] 
    possDots = []
    for l in indexes:
        if (len(p := posFromLocSet(pzl, l))) == 0 or len(p) == 1:
            if len(p) == 0:
                gStats["noChoices"] += 1
            return [p, l]
        if len(p) > gStats["maxChoices"]: 
            gStats["maxChoices"] = len(p)
            if len(p) == 9 and not hasMaxSym: 
                print("Positions maxChoices:", nbrs[loc])
                print("Symbols maxChoices:", p)
                hasMaxSym = True
        possDots.append((len(p), p, l))
    return min(possDots)[1:]

#return solved puzzle or empty string on failure
def bruteForce(pzl, loc):
    gStats["bruteForce"] += 1
    if isImpossibleLoc(pzl, loc): return "" #if puzzle fails, return an empty string
    if pzl.find(".") == -1: 
        return pzl #if puzzle is filled out(and correct), return the pzl

    possSym, dotIdx = findBestPos(pzl, loc)

    for ch in possSym: #all possible symbols
        subPzl = pzl[:dotIdx] + ch + pzl[dotIdx + 1:] #replaces best period with char
        bF = bruteForce(subPzl, dotIdx) #recursive call with subpzl
...
readFile(pzls)
#printPuzzle(".....7.95.....1...86..2.....2..73..85......6...3..49..3.5...41724................")
